...
| Method | Endpoint | Auth required | Request body | Response | Description |
|---|---|---|---|---|---|
| POST | `/patients` | Yes — PATIENT or ADMIN | `{ "userId": number, "dob": "YYYY-MM-DD", "address": "string", "medicalHistory": "string", "emergencyContact": "string" }` | `201 Created` — Patient profile | Creates a patient profile linked to a User from auth-service |
...
| Method | Endpoint | Auth required | Request body | Response | Description |
|---|---|---|---|---|---|
| POST | `/doctors` | Yes — DOCTOR or ADMIN | `{ "userId": number, "specialization": "string", "qualification": "string", "experienceYears": number, "consultationFee": number }` | `201 Created` — Doctor profile | Creates a doctor profile linked to a User |
