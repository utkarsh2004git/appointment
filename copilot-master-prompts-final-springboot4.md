...
- MapStruct
- ProblemDetail
- Verify dependencies before generating code.
- Use latest Spring APIs.
- Stop after each milestone and wait for confirmation.
...
```
Create a complete Spring Cloud Eureka Server module named eureka-server using Spring Boot 4.1.x and Spring Cloud 2025.1.x.
...
```
Create a complete Spring Cloud Gateway module named api-gateway using Spring Boot 4.1.x and Spring Cloud 2025.1.x.

IMPORTANT:
- Use Spring Cloud Gateway 5.x.
- Configure routes under 'spring.cloud.gateway.server.webflux.routes'.
- Never use 'spring.cloud.gateway.routes'.
- Ensure compatibility with Spring Boot 4.1.x and Spring Cloud 2025.1.x.

Include:
- pom.xml with spring-cloud-starter-gateway, eureka client, spring-boot-starter-security, jjwt (io.jsonwebtoken) 0.12.x (api, impl, jackson)
...
```
Create a complete Spring Boot microservice named auth-service using Spring Boot 4.1.x, Java 25.

Include:
- pom.xml with: spring-boot-starter-web, spring-boot-starter-data-jpa, org.postgresql:postgresql, spring-cloud-starter-netflix-eureka-client, spring-boot-starter-security, lombok, jjwt (io.jsonwebtoken) 0.12.x (api, impl, jackson) for api/impl/jackson
...
CONFIG:
- application.yml on port 8081, registering with Eureka at localhost:8761, PostgreSQL datasource pointing to schema auth_db, jwt.secret property, spring.jpa.hibernate.ddl-auto=validate
...
```
Create a complete Spring Boot microservice named patient-service using Spring Boot 4.1.x, Java 25.

Include:
- pom.xml with: spring-boot-starter-web, spring-boot-starter-data-jpa, org.postgresql:postgresql, spring-cloud-starter-netflix-eureka-client, lombok
...
CONFIG:
- application.yml on port 8082, Eureka registration, PostgreSQL schema patient_db, ddl-auto=validate
...
```
Create a complete Spring Boot microservice named doctor-service using Spring Boot 4.1.x, Java 25.
...
CONFIG:
- application.yml on port 8083, Eureka registration, PostgreSQL schema doctor_db, ddl-auto=validate
...
```
Create a complete Spring Boot microservice named appointment-service using Spring Boot 4.1.x, Java 25.

Include:
- pom.xml with: spring-boot-starter-web, spring-boot-starter-data-jpa, org.postgresql:postgresql, spring-cloud-starter-netflix-eureka-client, lombok, spring-cloud-starter-openfeign, spring-cloud-starter-circuitbreaker-resilience4j
...
CONFIG:
- application.yml on port 8084, Eureka registration, PostgreSQL schema appointment_db, ddl-auto=validate, Feign/Resilience4j timeout and circuit breaker settings (e.g., 3s timeout, 50% failure threshold)
...
```
Create a complete Spring Boot module named web-client using Spring Boot 4.1.x, Java 25, with Thymeleaf as the view layer. This module has NO database and NO entities — it only calls the API Gateway over HTTP and renders HTML.
